// File: lib/getDeviceDetails.js

/**
 * Detects: IP, Browser, OS, Device Type, Screen, Location
...
export async function getDeviceDetails() {
  ...
  const getOS = () => {
    ...
    if (/iPad/.test(ua))
      return `iPadOS ${ua.match(/CPU OS ([\d_]+)/)?.[1]?.replace(/_/g, ".") || ""}`;
    if (/Android/.test(ua))
  ...
  const getDeviceType = () => {
    ...
    if (
      /Mobile|iPhone|Android(?!.*Tablet)|BlackBerry|IEMobile|Opera Mini/.test(
        ua
      )
    )
      return "Mobile 📱";
  ...
  // ── IP + Location (with timeout + fallback) ──────────────
  let ip = "Unknown";
  let city = "Unknown";
  let region = "Unknown";
  let country = "Unknown";
  ...
  try {
    ...
    console.warn("Primary IP API failed:", err.message);
    ...
    try {
      ...
      console.warn("Fallback IP API failed:", fallbackErr.message);
    }
  }
  ...
  return {
    ...
    lon,
    location:
      city !== "Unknown" ? `${city}, ${region}, ${country}` : "Unknown",
    userAgent: ua,
  };
}