...
import {
  ...
  SortDesc, Download, Keyboard, Folder, MoreVertical,
  Edit2, CheckSquare, ChevronDown, Zap,
} from "lucide-react";
...
export default function NotesDashboard() {
  ...
  const [activeSection, setActiveSection] = useState("notes");  // notes | trash
  ...
  useEffect(() => { setIsDirty(true); }, [title, content, activeLabel, noteBg]);
  ...
  useHotkeys({
    ...
    "escape":      () => { setShowSearch(false); setShowLabels(false); setShowBgPicker(false); setShowSortMenu(false); setShowExport(false); setFocusMode(false); },
  });
  ...
  const deleteFolder = async (folderId, e) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        // Unset folderId on notes inside
  ...
  const saveNote = useCallback(async (auto = false) => {
    ...
    try {
      ...
      if (activeNote) {
        ...
        const participants = [user.uid, activeFolder].filter(Boolean);
        const ref = await addDoc(collection(db,"notes"), {
          userId:user.uid, folderId:activeFolder||null,
          ...payload, pinned:false, deleted:false,
          tags:[], createdAt:serverTimestamp(),
        });
        const newNote = { id:ref.id, ...payload, pinned:false, deleted:false, tags:[], folderId:activeFolder||null };
  ...
  const duplicateNote = useCallback(async () => {
    ...
    const ref = await addDoc(collection(db,"notes"), {
      ...
      pinned:false, deleted:false, tags:activeNote.tags||[],
  ...
  if (loading) return <div className={styles.loadingScreen}><span className={styles.loadingDot}/> Loading notes…</div>;
    ...
    <main className={`${styles.page} ${focusMode ? styles.focusMode : ""}`}>
      ...
      {showSearch && !focusMode && (
        <div className={styles.globalSearch}>
          ...
          <input autoFocus placeholder="Search by title, content or tag…" value={search} onChange={e=>setSearch(e.target.value)} className={styles.searchInput}/>
            ...
            <div className={styles.listHeader}>
              ...
              <div className={styles.listHeaderActions}>
                {showTrash && trashedNotes.length > 0 && (
                  <button className={styles.emptyTrashBtn} onClick={emptyTrash} title="Empty Trash"><Trash2 size={12}/></button>
                )}
                {!showTrash && (
                  <>
                    <button className={styles.newNoteBtn} onClick={() => setShowTemplates(true)} title="New from template"><Zap size={13}/></button>
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      ...
                      setActiveLabel(n.label||"none"); setNoteBg(n.noteBg||"default");
                      setIsNewNote(false); setIsDirty(false); setReadOnly(false);
                    }}
                  >
                      ...
                      <div className={styles.noteItemTop}>
                        ...
                        {!showTrash ? (
                          ...
                          <div className={styles.trashBtns}>
                            <button className={styles.restoreBtn} onClick={e=>{e.stopPropagation();restoreNote(n)}} title="Restore"><RotateCcw size={11}/></button>
                            <button className={styles.hardDeleteBtn} onClick={e=>{e.stopPropagation();hardDeleteNote(n)}} title="Delete forever"><X size={11}/></button>
                          </div>
                        )}
                      </div>
              ...
              <div className={styles.actions} style={{ background:isDarkBg?"rgba(0,0,0,0.2)":"", borderColor:isDarkBg?"rgba(255,255,255,0.1)":"" }}>
                ...
                {activeNote && !showTrash && (
                  <>
                    ...
                    <button className={styles.secondary} onClick={duplicateNote} title="Duplicate (Ctrl+D)"><Copy size={15}/> Duplicate</button>
