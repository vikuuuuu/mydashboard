...
const AI_FEATURES = [
  { id:"highlights", icon:"⚡", label:"Auto Highlights", desc:"Best moments" },
  { id:"silence",    icon:"🔇", label:"Remove Silence",  desc:"Dead air cut"  },
  { id:"noise",      icon:"🎙", label:"Noise Removal",   desc:"Audio clean"   },
  { id:"facetrack",  icon:"👤", label:"Face Tracking",   desc:"Auto-frame"    },
  { id:"autozoom",   icon:"🔍", label:"Auto Zoom",       desc:"Dynamic zoom"  },
  { id:"bgremove",   icon:"✂️", label:"BG Remove",       desc:"Remove BG"     },
];
const TOOL_TABS = [
...
export default function AIVideoEditor() {
  ...
  // Apply filter to native video element
  const filterStr = `brightness(${brightness}) contrast(${contrast}) saturate(${saturate}) hue-rotate(${hue}deg) grayscale(${grayscale}) sepia(${sepia})`;
  ...
  const loadVideo = (file) => {
    ...
    tmp.onloadedmetadata = () => {
      const d = Math.min(Math.floor(tmp.duration), 600);
      setVideoDur(d);
      setStartTime(0);
      setEndTime(d);
  ...
  const applyPreset = (idx) => {
    ...
    setSaturate(p.saturate);     setHue(p.hue);
    setGrayscale(p.grayscale);   setSepia(p.sepia);
  };

  // ── Subtitles ──
  const startAutoSub = () => {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SR) { alert("Chrome use karein subtitle ke liye."); return; }
    const r = new SR();
    ...
    r.onresult = (e) => {
      ...
      for (let i = e.resultIndex; i < e.results.length; i++) {
        ...
        if (e.results[i].isFinal) setSubLines(p => [...p, { start: Math.max(0, elapsed-2), end: elapsed, text: tx.trim() }]);
        else interim = tx;
      }
    ...
    r.start();
    recognRef.current = r;
    setSubListening(true); setSubLines([]); setCurrentSub("");
  };
  const stopAutoSub = () => { recognRef.current?.stop(); recognRef.current = null; setSubListening(false); };
  ...
  const simulateAI = (id) => {
    setAIProcessing(id);
    setTimeout(() => { setAIFeatures(p => ({...p, [id]: true})); setAIProcessing(null); }, 2000 + Math.random()*1500);
  };
  ...
  const buildSRT = (lines) => lines.map((l, i) => {
    const f = s => {
  ...
  const processVideo = async () => {
    ...
    try {
      ...
      setProgressMsg("Video load ho raha hai…"); setProgress(5);
      ...
      if (wmText) {
        const wx  = wmPos.includes("right") ? "w-text_w-20" : "20";
        const wy  = wmPos.includes("bottom") ? "h-text_h-20" : "20";
        const wes = wmText.replace(/'/g,"\\'").replace(/:/g,"\\:");
        vfArr.push(`drawtext=text='${wes}':fontcolor=white@${wmOpacity}:fontsize=20:x=${wx}:y=${wy}`);
      }
      const vf = vfArr.join(",") + subFilter;

      setProgressMsg("Processing video…"); setProgress(20);

      if (bgMusicFile) {
        await ff.writeFile("bgm.mp3", await fetchFile(bgMusicFile));
        await ff.exec(["-i","input.mp4","-i","bgm.mp3","-ss",`${startTime}`,"-t",`${endTime-startTime}`,
          ...
          "-map","[v]","-map","[a]","-c:v","libx264","-preset","ultrafast","-crf","22","-c:a","aac","-b:a","128k","-movflags","+faststart","-shortest","output.mp4"]);
      } else {
        await ff.exec(["-i","input.mp4","-ss",`${startTime}`,"-t",`${endTime-startTime}`,"-vf",vf,
          ...(muted ? ["-an"] : ["-af",`volume=${volume.toFixed(2)}`]),
          "-c:v","libx264","-preset","ultrafast","-crf","22","-c:a","aac","-b:a","128k","-movflags","+faststart","output.mp4"]);
      }

      setProgressMsg("Export ho raha hai…"); setProgress(90);
      const d = await ff.readFile("output.mp4");
      const outUrl = URL.createObjectURL(new Blob([d.buffer], { type:"video/mp4" }));
      setOutputUrl(outUrl);
      // Load exported video in player
      if (videoRef.current) { videoRef.current.src = outUrl; videoRef.current.load(); }
  ...
  const removeVideo = () => {
    if (videoUrl)  URL.revokeObjectURL(videoUrl);
    if (outputUrl) URL.revokeObjectURL(outputUrl);
    setVideoFile(null); setVideoUrl(""); setOutputUrl(null);
    setVideoDur(0); setStartTime(0); setEndTime(0);
    setClips([]); setPlayPos(0); setSubLines([]);
  };
    ...
    <div className={styles.app}>
      ...
      <header className={styles.topbar}>
        <div className={styles.topbarLeft}>
          ...
          {videoFile && (
            <div className={styles.fileChip}>
              ...
              <span>{videoFile.name.length > 22 ? videoFile.name.slice(0,20)+"…" : videoFile.name}</span>
              <button className={styles.fileChipClose} onClick={removeVideo} title="Remove video">✕</button>
            </div>
          )}
        </div>
        ...
        <div className={styles.topbarCenter}>
          {videoFile && (
            <div className={styles.quickStats}>
              ...
              <div className={styles.qDivider} />
              <div className={styles.qStat}>
                <span className={styles.qStatVal} style={{ color: clipDur > 60 ? "var(--warn)" : "var(--buy)" }}>
                  {clipDur > 60 ? "⚠️" : "✓"}
                </span>
        ...
        <div className={styles.topbarRight}>
          ...
          {outputUrl && (
            <a href={outputUrl} download="ai_short.mp4" className={styles.topDownloadBtn}>↓ Download</a>
          )}
        </div>
      </header>
      ...
      <div className={styles.layout}>
          ...
          {sidebarOpen && (<>
            ...
            <div className={styles.sideSection}>
              ...
              <div className={styles.transGrid}>
                {TRANSITIONS.map(t => (
                  <button key={t} className={`${styles.transBtn} ${transition===t?styles.transBtnActive:""}`}
                    onClick={() => setTransition(t)}>{t}</button>
                ))}
              </div>
            </div>
        ...
        <main className={styles.center}>
            ...
            >
              <div className={styles.uploadInner}>
                ...
                <div className={styles.uploadPills}>
                  {["AI Auto-Edit","Smart Subtitles","HD Export","FFmpeg Powered"].map(f => (
          ...
          {/* ── VIDEO PREVIEW (native <video> element — no ReactPlayer issues) ── */}
          {videoFile && (
            <div className={styles.previewArea}>
              <div className={styles.previewWrap}>
                {/* Native HTML5 video — always works, no height collapse */}
                <video
                  ref={videoRef}
                ...
                {(activeSub || (subListening && currentSub)) && (
                  <div
                    ...
                    style={{ fontSize: subFontSize, color: subColor, bottom: 52 }}
                  >
                ...
                {/* Watermark Preview */}
                {wmText && (
                  <div className={`${styles.wmOverlay} ${styles[`wm_${wmPos.replace("-","_")}`]}`} style={{ opacity: wmOpacity }}>
                    {wmText}
                  </div>
                )}
                ...
                {/* Change file */}
                <button className={styles.changeFileBtn} onClick={() => fileInputRef.current?.click()} title="Change video">🔄</button>
                <input ref={fileInputRef} type="file" accept="video/*" hidden onChange={e => loadVideo(e.target.files[0])} />
              </div>
            </div>
          )}

          {/* ── TIMELINE ── */}
          {videoFile && videoDur > 0 && (
            <div className={styles.timelineSection}>
              <div className={styles.tlHeader}>
                ...
                <div className={styles.tlActions}>
                  {[["15s",15],["30s",30],["60s",60],["Full",videoDur]].map(([l,v]) => (
                    <button key={l} className={styles.tlBtn}
                      onClick={() => { setStartTime(0); setEndTime(Math.min(v, videoDur)); }}>{l}</button>
                  ))}
                </div>
              </div>
              ...
              <div className={styles.trackOuter}>
                ...
                <div className={styles.clipsRow}>
                  {clips.map((clip,i) => (
                    <div key={clip.id} className={styles.clipBlock}
                      style={{ left:`${(clip.start/videoDur)*100}%`, width:`${((clip.end-clip.start)/videoDur)*100}%` }}>
          ...
          <div className={styles.tabContent}>
            ...
            {activeTab === "trim" && (
              <div className={styles.tabPane}>
                ...
                <div className={styles.field}>
                  ...
                  <input type="range" min={0} max={Math.max(videoDur-1,0)} value={startTime}
                    onChange={e => setStartTime(Math.min(+e.target.value, endTime-1))}
                    className={styles.slider} style={{"--pct":`${videoDur>0?(startTime/videoDur)*100:0}%`}} />
                  <span className={styles.sliderVal}>{fmtTime(startTime)}</span>
                </div>
                <div className={styles.field}>
                  <label className={styles.fieldLabel}>End Time</label>
                  <input type="range" min={1} max={videoDur||1} value={endTime}
                    onChange={e => setEndTime(Math.max(+e.target.value, startTime+1))}
                    className={styles.slider} style={{"--pct":`${videoDur>0?(endTime/videoDur)*100:100}%`}} />
            ...
            {activeTab === "filters" && (
              <div className={styles.tabPane}>
                ...
                <div className={styles.filterScroll}>
                  {FILTER_PRESETS.map((p,i) => (
                    <div key={p.name}
                      ...
                      <div className={styles.filterSwatch}
                ...
                <button className={styles.resetBtn} onClick={() => applyPreset(0)}>↩ Reset Filters</button>
              </div>
            )}
            ...
            {activeTab === "text" && (
              <div className={styles.tabPane}>
                ...
                <div className={styles.field}>
                  ...
                  <input className={styles.textInput} type="text" placeholder="Follow for more! 🔥"
                ...
                <div className={styles.field}>
                  ...
                  <input type="range" min={14} max={90} step={1} value={fontSize}
                    ...
                    className={styles.slider} style={{"--pct":`${((fontSize-14)/76)*100}%`}} />
                </div>
                <div className={styles.field}>
                ...
                <div className={styles.field}>
                  ...
                  <div className={styles.posGrid}>
                    {[["top","⬆ Top"],["center","⬛ Center"],["bottom","⬇ Bottom"]].map(([p,l]) => (
                      <button key={p} className={`${styles.posBtn} ${textPos===p?styles.posBtnActive:""}`}
                        onClick={() => setTextPos(p)}>{l}</button>
                    ))}
                  </div>
                </div>
            ...
            {activeTab === "subtitles" && (
              <div className={styles.tabPane}>
                ...
                <div className={styles.infoBox}>🎤 Video play karein, fir Record karein — subtitles auto-sync honge</div>
                <div className={styles.subControls}>
                  <button className={`${styles.recBtn} ${subListening?styles.recBtnActive:""}`}
                ...
                <div className={styles.field}>
                  ...
                  <input type="range" min={14} max={60} step={1} value={subFontSize}
                    ...
                    className={styles.slider} style={{"--pct":`${((subFontSize-14)/46)*100}%`}} />
                </div>
                <div className={styles.field}>
                  <label className={styles.fieldLabel}>Color</label>
                  <div className={styles.colorRow}>
                ...
                <div className={styles.audioMixRow}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <input type="range" min={0} max={1} step={0.01} value={muted?0:volume}
                      ...
                      className={styles.slider} style={{"--pct":`${(muted?0:volume)*100}%`}} />
                  </div>
                </div>
                <div className={styles.toggleRow}>
                ...
                {bgMusicFile && (
                  <div className={styles.field}>
                    ...
                    <input type="range" min={0} max={1} step={0.01} value={bgMusicVol}
                      ...
                      className={styles.slider} style={{"--pct":`${bgMusicVol*100}%`}} />
                  </div>
                )}
                <div className={styles.infoBox}>💡 AI Noise Removal sidebar mein available hai</div>
              </div>
            )}
              ...
              <div className={styles.tabPane}>
                ...
                <div className={styles.field}>
                  ...
                  <div className={styles.wmPosGrid}>
                    {[["top-left","↖"],["top-right","↗"],["bottom-left","↙"],["bottom-right","↘"],["center","⊕"]].map(([p,icon]) => (
                      <button key={p} className={`${styles.wmPosBtn} ${wmPos===p?styles.wmPosBtnActive:""}`}
                        onClick={() => setWmPos(p)}>
                ...
                <div className={styles.field}>
                  <div className={styles.fieldRow}>
                    ...
                    <span className={styles.sliderVal}>{Math.round(wmOpacity*100)}%</span>
                  </div>
                  <input type="range" min={0.1} max={1} step={0.05} value={wmOpacity}
                    onChange={e => setWmOpacity(+e.target.value)}
                    className={styles.slider} style={{"--pct":`${wmOpacity*100}%`}} />
                </div>
                {wmText && (
                  <div className={styles.wmPreview}>
                    <span style={{opacity:wmOpacity}}>Preview: {wmText}</span>
                  </div>
                )}
              </div>
            )}
              ...
              <div className={styles.tabPane}>
                ...
                <div className={styles.checklist}>
                  ...
                  {[
                    ...
                    { ok:subLines.length>0,                       label:"Subtitles",  opt:true },
                ...
                {outputUrl && !isProcessing && (
                  <a href={outputUrl} download="ai_short.mp4" className={styles.downloadBtn}>⬇ Download Video</a>
                )}
              </div>
            )}
