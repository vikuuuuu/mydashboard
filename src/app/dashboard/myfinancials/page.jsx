...
export default function FinancialsPage() {
  ...
  const holdings = useMemo(() => {
    ...
    transactions.forEach((tx) => {
      ...
      const s = map[tx.symbol];
      s.totalCharges += charges;
      if (tx.action === "BUY") {
        s.lots.push({ qty, price });
        s.qty += qty;
        s.invested += qty * price;
      }
      if (tx.action === "SELL") {
        let rem = qty;
        while (rem > 0 && s.lots.length > 0) {
          const lot = s.lots[0];
          if (lot.qty <= rem) {
            s.realized += lot.qty * (price - lot.price);
            s.invested -= lot.qty * lot.price;
            rem -= lot.qty;
            s.qty -= lot.qty;
            s.lots.shift();
          } else {
            s.realized += rem * (price - lot.price);
            s.invested -= rem * lot.price;
            lot.qty -= rem;
            s.qty -= rem;
            rem = 0;
          }
  ...
  const khataStats = useMemo(() => {
    const totalExpense = khataEntries
      .filter((e) => e.type === "EXPENSE")
      .reduce((s, e) => s + Number(e.amount), 0);
    const totalGiven = khataEntries
      .filter((e) => e.type === "PAYMENT_GIVEN")
      .reduce((s, e) => s + Number(e.amount), 0);
    const totalReceived = khataEntries
    ...
    khataEntries
      ...
      .forEach((e) => {
        ...
        if (!byContact[e.contactName])
          byContact[e.contactName] = { given: 0, received: 0 };
        if (e.type === "PAYMENT_GIVEN")
          byContact[e.contactName].given += Number(e.amount);
    ...
    khataEntries.forEach((e) => {
      ...
      else if (e.type === "PAYMENT_GIVEN")
        byMonth[key].given += Number(e.amount);
  ...
  const saveStock = async (e) => {
    e.preventDefault();
    setStockFormError("");
    const symbol = stockForm.symbol.toUpperCase(),
      qty = Number(stockForm.qty),
      price = Number(stockForm.price);
    if (stockForm.action === "SELL") {
      const h = holdings.find((h) => h.symbol === symbol);
      if (!h || qty > h.qty) {
        setStockFormError("Not enough stock to sell!");
        return;
      }
    }
    const chargePayload = {};
  ...
  const saveKhata = async (e) => {
    e.preventDefault();
    const payload = {
      type: khataForm.type,
      amount: Number(khataForm.amount),
      category: khataForm.category,
      description: khataForm.description,
      contactName: khataForm.contactName,
  ...
  const editKhata = (e) => {
    setKhataForm({
      ...
      date: e.createdAt?.seconds
        ? new Date(e.createdAt.seconds * 1000).toISOString().slice(0, 16)
        : e.createdAt,
      id: e.id,
    });
  ...
  const formatDate = (ca) => {
    const d = new Date(ca?.seconds ? ca.seconds * 1000 : ca);
  ...
  transactions.forEach((t) => {
    ...
    if (!byMonthStock[k]) byMonthStock[k] = { buy: 0, sell: 0 };
    const v = Number(t.qty) * Number(t.price);
    if (t.action === "BUY") byMonthStock[k].buy += v;
    else byMonthStock[k].sell += v;
  });
  const chargeKeys = [
    "brokerage",
    "exchangeCharges",
  ...
  const chargeFiltered = chargeKeys
    .map((k, i) => ({
      label: chargeLabels[i],
      val: transactions.reduce((s, t) => s + Number(t[k] || 0), 0),
    ...
    <div className={styles.container}>
      ...
      <div className={styles.moduleSwitcher}>
        ...
        >
          ...
          <div>
            <div className={styles.moduleBtnTitle}>Portfolio Manager</div>
            <div className={styles.moduleBtnSub}>
              Stocks · P&L · Holdings · Charts
            </div>
          </div>
        </button>
        <button
        ...
        >
          <span className={styles.moduleIcon}>📒</span>
          <div>
            <div className={styles.moduleBtnTitle}>Khata Book</div>
            <div className={styles.moduleBtnSub}>
              Expenses · Payments · Ledger · Analytics
            </div>
          </div>
        </button>
      </div>
        ...
        <>
            ...
            {[
              {
                ...
                value: `₹${totalInvest.toLocaleString("en-IN")}`,
                mod: "",
              },
              {
                label: "Current Value",
                value: `₹${totals.currentInvest.toLocaleString("en-IN")}`,
                mod: "",
              },
              {
                label: "Realised P&L",
                value: `${totals.totalPL >= 0 ? "+" : ""}₹${totals.totalPL.toLocaleString("en-IN")}`,
                mod:
                  totals.totalPL >= 0
                    ? styles.cardPositive
                    : styles.cardNegative,
                color: totals.totalPL >= 0 ? "var(--buy)" : "var(--sell)",
              },
              {
                label: "Total Charges",
                value: `−₹${totalChargesAll.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`,
                mod: styles.cardCharges,
                color: "var(--charges)",
              },
              {
                label: "Net P&L (after charges)",
                value: `${netPL >= 0 ? "+" : ""}₹${netPL.toLocaleString("en-IN")}`,
                mod: netPL >= 0 ? styles.cardPositive : styles.cardNegative,
                color: netPL >= 0 ? "var(--buy)" : "var(--sell)",
              },
              { label: "Active Positions", value: totals.totalStocks, mod: "" },
            ].map(({ label, value, mod, color }) => (
              <div key={label} className={`${styles.card} ${mod}`}>
                <span>{label}</span>
          ...
          <div className={styles.chartsGrid}>
            ...
            <div className={`${styles.chartCard} ${styles.chartCardWide}`}>
              ...
              {Object.keys(byMonthStock).length > 0 ? (
                <div className={`${styles.chartWrap} ${styles.chartWrapLg}`}>
                  <Line
                    data={{
                      labels: Object.keys(byMonthStock),
                      datasets: [
                        {
                          label: "Buy",
                          data: Object.values(byMonthStock).map((m) => m.buy),
                          borderColor: "#0f9d6e",
                          ...
                          borderWidth: 2,
                          pointRadius: 4,
                          pointBackgroundColor: "#0f9d6e",
                        },
                        {
                          label: "Sell",
                          data: Object.values(byMonthStock).map((m) => m.sell),
                          borderColor: "#e63946",
            ...
            <div className={styles.chartCard}>
              ...
              {chargeFiltered.length > 0 ? (
                ...
                <div className={styles.chartEmpty}>No charges</div>
              )}
            </div>
            <div className={styles.chartCard}>
                  ...
                  return (
                    <div key={h.symbol} className={styles.holdingCard}>
                      ...
                      <div className={styles.holdingMeta}>
                        ...
                        <div
                          className={`${styles.holdingPL} ${h.realized >= 0 ? styles.holdingPLPos : styles.holdingPLNeg}`}
                        >
                          {h.realized >= 0 ? "+" : ""}₹
                          {h.realized.toLocaleString("en-IN", {
                            minimumFractionDigits: 2,
                          })}
                          <div className={styles.holdingPLLabel}>
                            Realized P&L
                          </div>
                        </div>
                      </div>
                      ...
                      <div className={styles.holdingBar}>
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, (h.invested / maxInvested) * 100)}%`,
                          }}
                        />
                      </div>
                    </div>
        ...
        <>
          <div className={styles.kpiGrid}>
            <div className={`${styles.card} ${styles.cardNegative}`}>
              ...
              <h2 style={{ color: "var(--sell)" }}>
                ₹{khataStats.totalExpense.toLocaleString("en-IN")}
              </h2>
            </div>
            <div className={`${styles.card} ${styles.cardCharges}`}>
              <span>Payment Given</span>
              <h2 style={{ color: "var(--charges)" }}>
                ₹{khataStats.totalGiven.toLocaleString("en-IN")}
              </h2>
            </div>
            <div className={`${styles.card} ${styles.cardPositive}`}>
              <span>Payment Received</span>
              <h2 style={{ color: "var(--buy)" }}>
                ₹{khataStats.totalReceived.toLocaleString("en-IN")}
              </h2>
            </div>
            <div
              className={`${styles.card} ${khataStats.netBalance >= 0 ? styles.cardPositive : styles.cardNegative}`}
            >
              <span>Net Balance (Recv − Given)</span>
              <h2
                style={{
                  color:
                    khataStats.netBalance >= 0 ? "var(--buy)" : "var(--sell)",
                }}
              >
                {khataStats.netBalance >= 0 ? "+" : ""}₹
                {khataStats.netBalance.toLocaleString("en-IN")}
              </h2>
            </div>
            <div className={styles.card}>
              <span>Total Entries</span>
              <h2>{khataEntries.length}</h2>
            </div>
            <div className={styles.card}>
              <span>Contacts</span>
              <h2>{Object.keys(khataStats.byContact).length}</h2>
            </div>
          </div>
          <button
            className={styles.addTradeBtn}
          ...
          <div className={styles.sectionHeader}>
            <span>📊 Khata Analytics</span>
          </div>
          <div className={styles.chartsGrid}>
            <div className={styles.chartCard}>
              <div className={styles.chartTitle}>Expense by Category</div>
              {Object.keys(khataStats.byCat).length > 0 ? (
                <>
                  <div className={styles.chartWrap}>
                    <Doughnut
                      data={{
                        ...
                        datasets: [
                          {
                            data: Object.values(khataStats.byCat),
                  ...
                  <div className={styles.chartLegend}>
                    {Object.keys(khataStats.byCat).map((k) => {
                      const cat = EXPENSE_CATEGORIES.find((c) => c.id === k);
                      return (
                        <span key={k} className={styles.legendItem}>
                          <span
                            className={styles.legendDot}
                            style={{ background: cat?.color || "#aaa" }}
                          />
                          {cat?.label?.split(" ").slice(1).join(" ") || k}
                        </span>
                      );
                    })}
                  </div>
                ...
                <div className={styles.chartEmpty}>No expenses yet</div>
              )}
            </div>
            <div className={`${styles.chartCard} ${styles.chartCardWide}`}>
              <div className={styles.chartTitle}>
                Monthly Overview — Expense / Given / Received
              </div>
              {Object.keys(khataStats.byMonth).length > 0 ? (
                <div className={`${styles.chartWrap} ${styles.chartWrapLg}`}>
                  <Line
                    data={{
                      labels: Object.keys(khataStats.byMonth),
                      datasets: [
                        {
                          label: "Expense",
            ...
            {contactList.length > 0 && (
              <div className={`${styles.chartCard} ${styles.chartCardWide}`}>
                <div className={styles.chartTitle}>
                  Contact-wise Payment Ledger
                </div>
                <div className={`${styles.chartWrap} ${styles.chartWrapLg}`}>
                  <Bar
                    data={{
                      labels: contactList.map(([n]) => n),
                      datasets: [
                        {
                          label: "Given",
                          data: contactList.map(([, v]) => v.given),
                          backgroundColor: "rgba(230,57,70,0.7)",
                          borderRadius: 5,
                        },
                        {
                          label: "Received",
                          data: contactList.map(([, v]) => v.received),
                          backgroundColor: "rgba(15,157,110,0.7)",
                          borderRadius: 5,
                        },
                      ],
                    }}
            ...
            <>
              ...
              <div className={styles.holdingsGrid}>
                {contactList.map(([name, v]) => {
                  ...
                  return (
                    <div key={name} className={styles.holdingCard}>
                      <div className={styles.holdingSymbol}>👤 {name}</div>
                      <div
                        className={styles.holdingMeta}
                        style={{ marginTop: 10 }}
                      >
                        <div>
                          <div className={styles.holdingQtyLabel}>Given</div>
                          <div
                            className={styles.holdingQtyVal}
                            style={{ color: "var(--sell)", fontSize: 15 }}
                          >
                            ₹{v.given.toLocaleString("en-IN")}
                          </div>
                        </div>
                        <div style={{ textAlign: "right" }}>
                          <div className={styles.holdingQtyLabel}>Received</div>
                          <div
                            className={styles.holdingQtyVal}
                            style={{ color: "var(--buy)", fontSize: 15 }}
                          >
                      ...
                      <div
                        className={styles.holdingInvested}
                        style={{ marginTop: 8 }}
                      >
                        Net:{" "}
                        <strong
                          style={{
                            color: net >= 0 ? "var(--buy)" : "var(--sell)",
                          }}
                        >
      ...
      {stockModalOpen && (
        <div
          ...
          onClick={(e) => {
            if (
              stockModalRef.current &&
              !stockModalRef.current.contains(e.target)
            )
              closeStockModal();
          }}
        >
          <div className={styles.modal} ref={stockModalRef}>
            <div className={styles.modalHeader}>
              <span className={styles.modalTitle}>
                {stockForm.id ? "✏️ Edit Trade" : "➕ Add Trade"}
              </span>
            ...
            {stockFormError && (
              <div className={styles.formAlert}>{stockFormError}</div>
            )}
            <form onSubmit={saveStock}>
              <div className={styles.formRow}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setStockForm({ ...stockForm, stockType: e.target.value })
                    }
                  >
              ...
              <div className={styles.chargesDivider}>
                <span
                  className={
                    stockForm.action === "BUY"
                      ? styles.chargesLabelBuy
                      : styles.chargesLabelSell
                  }
                >
                  💸{" "}
                  {stockForm.action === "BUY" ? "BUY Charges" : "SELL Charges"}{" "}
                  (optional)
                </span>
              </div>
              ...
              <div className={styles.formRow}>
                {CHARGE_FIELDS[stockForm.action].map((field) => (
                ...
                {formChargesTotal > 0 && (
                  <div className={styles.chargesPreview}>
                    ...
                    <strong>
                      ₹
                      {formChargesTotal.toLocaleString("en-IN", {
                        minimumFractionDigits: 2,
                      })}
                    </strong>
                  </div>
              ...
              <div className={styles.formActions}>
                ...
                <button type="submit" className={styles.submitBtn}>
                  {stockForm.id ? "Update Trade" : "Add Trade"}
                </button>
              </div>
            </form>
          </div>
      ...
      {khataModalOpen && (
        <div
          ...
          onClick={(e) => {
            if (
              khataModalRef.current &&
              !khataModalRef.current.contains(e.target)
            )
              closeKhataModal();
          }}
        >
          <div className={styles.modal} ref={khataModalRef}>
            <div className={styles.modalHeader}>
              <span className={styles.modalTitle}>
                {khataForm.id ? "✏️ Edit Entry" : "➕ Add Khata Entry"}
              </span>
              <button className={styles.modalClose} onClick={closeKhataModal}>
                ✕
              </button>
            </div>
            <form onSubmit={saveKhata}>
                ...
                {[
                  {
                    val: "EXPENSE",
                    label: "💸 Expense",
                    cls: styles.khataTypeExpense,
                  },
                  {
                    val: "PAYMENT_GIVEN",
                    label: "🔴 Payment Given",
                    cls: styles.khataTypeGiven,
                  },
                  {
                    val: "PAYMENT_RECEIVED",
                    label: "🟢 Payment Received",
                    cls: styles.khataTypeReceived,
                  },
                ].map(({ val, label, cls }) => (
                  <button
                    type="button"
                    key={val}
                    className={`${styles.khataTypeBtn} ${khataForm.type === val ? cls : ""}`}
                    onClick={() => setKhataForm({ ...khataForm, type: val })}
                  >
                    {label}
                  </button>
                ))}
              ...
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setKhataForm({ ...khataForm, amount: e.target.value })
                    }
                    required
                  />
                </div>
                ...
                {khataForm.type === "EXPENSE" && (
                  <div className={styles.formGroup}>
                    ...
                    <select
                      value={khataForm.category}
                      onChange={(e) =>
                        setKhataForm({ ...khataForm, category: e.target.value })
                      }
                    >
                ...
                {(khataForm.type === "PAYMENT_GIVEN" ||
                  khataForm.type === "PAYMENT_RECEIVED") && (
                  <div className={styles.formGroup}>
                    <label>Contact Name</label>
                    <input
                      placeholder="Person / Company name"
                      value={khataForm.contactName}
                      onChange={(e) =>
                        setKhataForm({
                          ...khataForm,
                ...
                <div className={styles.formGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setKhataForm({ ...khataForm, date: e.target.value })
                    }
                  />
                </div>
                <div className={`${styles.formGroup} ${styles.formGroupWide}`}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setKhataForm({ ...khataForm, notes: e.target.value })
                    }
                  />
                </div>
              </div>
              <div className={styles.formActions}>
                ...
                <button type="submit" className={styles.submitBtn}>
                  {khataForm.id ? "Update Entry" : "Add Entry"}
                </button>
              </div>
            </form>
          </div>
...
function StockTable({
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      let va, vb;
      if (sortKey === "date") {
        va = a.createdAt?.seconds || new Date(a.createdAt).getTime() / 1000;
        vb = b.createdAt?.seconds || new Date(b.createdAt).getTime() / 1000;
        ...
        va = Number(a.qty) * Number(a.price);
        vb = Number(b.qty) * Number(b.price);
      }
            ...
            <tr>
              ...
              {(tab === "ALL" || tab === "BUY") && (
                <>
                  ...
                  <th className={styles.thBuy}>Exch.</th>
                  <th className={styles.thBuy}>GST</th>
                  <th className={styles.thBuy}>Stamp</th>
                </>
              )}
              {(tab === "ALL" || tab === "SELL") && (
                <>
                  ...
                  <th className={styles.thSell}>GST</th>
                </>
              )}
              <th className={styles.thCharges}>Charges</th>
              <th className={styles.thFinal}>Net</th>
              ...
              return (
                <tr key={i}>
                  ...
                  {tab === "ALL" && (
                    <td>
                      <span
                        className={isBuy ? styles.badgeBuy : styles.badgeSell}
                      >
                        {t.action}
                      </span>
                    </td>
                  ...
                  <td className={styles.monoCell}>
                    ₹{tv.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                  </td>
                  {(tab === "ALL" || tab === "BUY") && (
                    <>
                      <td className={`${styles.monoCell} ${styles.buyCol}`}>
                        {isBuy ? (
                          ...
                          <span className={styles.naCell}>N/A</span>
                        )}
                      </td>
                      <td className={`${styles.monoCell} ${styles.buyCol}`}>
                        {isBuy ? (
                          fmtVal(t.exchangeCharges) || (
                            <span className={styles.nilCell}>—</span>
                          )
                        ) : (
                          <span className={styles.naCell}>N/A</span>
                  ...
                  <td className={`${styles.monoCell} ${styles.chargesCell}`}>
                    {ch > 0 ? (
                      `₹${ch.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`
                    ) : (
                      <span className={styles.nilCell}>—</span>
                    )}
                  </td>
      ...
      <div className={styles.tableTopBar}>
        <div className={styles.tabGroup}>
          <button
            ...
            onClick={() => setTab("ALL")}
          >
            All <span className={styles.tabCount}>{entries.length}</span>
          </button>
          <button
            className={`${styles.tabBtn} ${tab === "EXPENSE" ? styles.tabExpenseActive : ""}`}
          ...
          <button
            className={`${styles.tabBtn} ${tab === "PAYMENT_GIVEN" ? styles.tabSellActive : ""}`}
            onClick={() => setTab("PAYMENT_GIVEN")}
          >
            🔴 Given{" "}
            <span className={styles.tabCount}>
              {entries.filter((e) => e.type === "PAYMENT_GIVEN").length}
            </span>
          </button>
          <button
            className={`${styles.tabBtn} ${tab === "PAYMENT_RECEIVED" ? styles.tabBuyActive : ""}`}
            onClick={() => setTab("PAYMENT_RECEIVED")}
          >
            🟢 Received{" "}
            <span className={styles.tabCount}>
              {entries.filter((e) => e.type === "PAYMENT_RECEIVED").length}
            </span>
          </button>
        </div>
        <button className={styles.exportBtn} onClick={exportCSV}>
            ...
            {filtered.map((e, i) => {
              ...
              return (
                <tr key={i}>
                  ...
                  <td>
                    {e.contactName || <span className={styles.nilCell}>—</span>}
                  </td>
                  <td style={{ color: "var(--text2)", fontSize: 12 }}>
                    {e.notes || <span className={styles.nilCell}>—</span>}
                  </td>
