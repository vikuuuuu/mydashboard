...
function use360(canvasRef, src, active) {
  const S = useRef({
    ...
    drag: false, lx: 0, ly: 0, vx: 0, vy: 0,
    auto: false, gyro: false, _gh: null, animId: null,
  });
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      gl.useProgram(pg);
      gl.uniform1i(ul('t'), 0);
      gl.uniform1f(ul('Y'), s.yaw);
      gl.uniform1f(ul('P'), s.pitch);
      gl.uniform1f(ul('F'), s.fov);
  ...
  const onW  = useCallback(e => {
    e.preventDefault();
    S.current.fov = Math.max(20, Math.min(110, S.current.fov + e.deltaY * 0.05));
  }, []);
  ...
  const enableGyro  = useCallback(async () => {
    ...
    const h = e => {
      if (!S.current.gyro) return;
      S.current.yaw   = -(e.alpha || 0) * Math.PI / 180;
      S.current.pitch  = (e.beta  || 0) * Math.PI / 180 * 0.5;
    };
    ...
    <div className={styles.dg}>
      ...
      <div className={styles.dgMap}>
        {SHOTS.filter(s => s.row === 1).map(s => {
          ...
          return (
            <div key={s.idx}
              className={`${styles.dgDot} ${isDone ? styles.dgDone : ''} ${isAct ? styles.dgActive : ''}`}
      ...
      <div className={styles.dgInfo}>
        ...
        {hasGyro ? (
          ...
          <div className={styles.dgNoGyro}>No gyroscope — manually aim camera → tap shutter</div>
        )}
        <div className={styles.dgLevels}>
          {LEVELS.map((v, vi) => {
            const c = SHOTS.filter(s => s.row === vi && done.has(s.idx)).length;
            return (
              <div key={vi} className={`${styles.dgPill} ${vi === shot.row ? styles.dgPillOn : ''}`}>
                {v.icon} {v.label} {c}/{H_COUNT}
              </div>
            );
          })}
        </div>
...
function Viewer({ src, onClose }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      if (S.current) setCmp((((S.current.yaw * 180 / Math.PI) % 360) + 360) % 360);
    }, 100);
  ...
  const doGyro = async () => {
    if (gy) { disableGyro(); setGy(false); }
    else    { const ok = await enableGyro(); setGy(ok); if (!ok) alert('Gyroscope unavailable or permission denied.'); }
  };
  const doFS = () => {
    if (!document.fullscreenElement) wRef.current?.requestFullscreen?.().catch(() => {});
      ...
      >
        ...
        <div className={styles.compass}>
          <div className={styles.cmpRing} style={{ transform: `rotate(${-cmp}deg)` }}>
            <span className={styles.cN}>N</span>
            <span className={styles.cS}>S</span>
            <span className={styles.cE}>E</span>
            <span className={styles.cW}>W</span>
          </div>
        ...
        <div className={styles.vCtrl}>
          <button className={styles.vcb} onClick={resetView} title="Reset view">⌖</button>
        ...
        <div className={styles.vHint}>Drag to look · Scroll to zoom · 📡 gyro</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className={styles.gc}>
      <div className={styles.gcThumb} onClick={() => onView(p)}>
        {p.url
          ? <img src={p.url} alt={p.name} className={styles.gcImg} loading="lazy" />
          : <div className={styles.gcNone}>🌐</div>
        }
      ...
      <div className={styles.gcBody}>
        {ed ? (
          <div className={styles.rnRow}>
            <input className={styles.rnIn} value={nv}
          ...
          <div className={styles.gcName} onDoubleClick={() => setEd(true)} title="Double-click to rename">
            {p.name}
          </div>
        )}
        ...
        <div className={styles.gcActs}>
          ...
          {cf
            ? <>
                <button className={styles.gBtnR} onClick={() => onDel(p)}>Confirm</button>
                <button className={styles.gBtn}  onClick={() => setCf(false)}>Cancel</button>
              </>
...
export default function PanoramaPage() {
  ...
  const savePano = async () => {
    ...
    setSaving(true);
    try {
      const ref = await addDoc(collection(db, `users/${uid}/panoramas`), {
        name, shots: cap.done.size, createdAt: serverTimestamp(),
      });
          ...
          {loading ? (
            ...
            <div className={styles.empty}>
              ...
              <p className={styles.emptySub}>
                Capture a 360° panorama or upload an equirectangular image.
                All images stored locally — no cloud storage needed.
              </p>
              <div className={styles.emptyActs}>
                <button className={styles.pri} onClick={() => setTab('capture')}>📷 Capture Now</button>
                <label className={styles.sec}>
                  ⬆ Upload
                  <input type="file" accept="image/*" style={{ display: 'none' }} onChange={uploadFile} />
                </label>
              </div>
          ...
          <div className={styles.capWrap}>
            ...
            <div className={`${styles.vidBox} ${camActive ? styles.vidBoxVisible : styles.vidBoxHidden}`}>
              ...
              {cap.step === 'capturing' && (
                <div className={styles.vidOvl}>
                  ...
                  <div className={styles.lvlLbl}>
                    <span>{curShot.icon}</span>
                    <span>{curShot.label}</span>
                  </div>
            ...
            {cap.step === 'camStarting' && (
              <div className={styles.camStarting}>
                ...
                <p>Starting camera…</p>
                <p className={styles.camStartSub}>Please allow camera permission when prompted</p>
              </div>
            )}
            ...
            {cap.step === 'guide' && (
              <div className={styles.guideBtns} style={{ marginTop: 12 }}>
                <button className={styles.pri} onClick={cap.startCapturing}>
                  ● Start Guided Capture ({TOTAL} shots)
                </button>
      ...
      {tab === 'viewer' && viewing && (
        <div className={styles.viewPage}>
          <div className={styles.viewHdr}>
            ...
            <button className={styles.sec} onClick={() => { setTab('gallery'); setViewing(null); }}>
              ← Gallery
            </button>
          </div>
